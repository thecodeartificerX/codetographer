 ...
 */
function parseRoutingRules(indexContent) {
    const mappings = [];
 ...
 */
export function matchDomains(text, indexPath) {
    ...
    for (const mapping of mappings) {
        ...
        if (score > 0)
            scores.set(mapping.domain, score);
    }
    if (scores.size === 0)
        return [];
...
export function getDomainDocPath(projectDir, domain) {
