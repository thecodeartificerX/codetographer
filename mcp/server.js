...
function getDomainForFile(file, mappings) {
    const fwd = file.replace(/\\/g, '/');
    for (const m of mappings) {
        for (const p of m.paths) {
            if (fwd.startsWith(p) || fwd.includes(p))
                return m.domain;
        }
    }
    return undefined;
    ...
    for (const tag of allTags) {
        ...
        for (const kw of keywords) {
            if (nameLower === kw)
                ...
                score += 2;
            else if (tag.signature?.toLowerCase().includes(kw))
                score += 3;
        }
        if (score > 0) {
...
async function main() {
    ...
    server.registerTool('codetographer_search', {
        ...
        const text = results.map(r => `${r.file}:${r.line} [${r.kind}] ${r.name}${r.domain ? ` (${r.domain})` : ''}\n  ${r.signature ?? ''}`).join('\n\n');
        return { content: [{ type: 'text', text }] };
    });
    server.registerTool('codetographer_domain', {
    ...
    server.registerTool('codetographer_status', {
