...
export interface SanityOptions {
  /** Target project root (CLAUDE_PROJECT_DIR) */
  projectDir: string;
  /** Plugin install dir (CLAUDE_PLUGIN_ROOT) */
  pluginRoot: string;
  /** Writable plugin data dir with node_modules (CLAUDE_PLUGIN_DATA) */
  pluginData: string;
  /** Attempt auto-fixes (default: true) */
  fix: boolean;
  /** Rendering hint: suppress 'pass' checks in CLI output (default: false) */
  quiet: boolean;
  /** Skip map regeneration — use true in session-start (default: false) */
 ...
 */
function parseDomainNames(indexContent: string): string[] {
  ...
  for (const line of lines) {
    ...
    if (!inDomainMapSection) continue;
 ...
 */
function checkPluginRootEnv(pluginRoot: string): SanityCheckResult {
 ...
 */
function checkPluginDataEnv(pluginData: string): SanityCheckResult {
 ...
 */
function checkProjectDirEnv(): SanityCheckResult {
 ...
 */
function checkNodeModules(pluginRoot: string, pluginData: string, fix: boolean): SanityCheckResult {
 ...
 */
function checkHookScripts(pluginRoot: string): SanityCheckResult {
 ...
 */
function checkDocsDir(projectDir: string, fix: boolean): SanityCheckResult {
 ...
 */
function checkIndexMd(projectDir: string): SanityCheckResult {
  const name = 'INDEX.md';
 ...
 */
function checkChangesMd(projectDir: string, fix: boolean): { result: SanityCheckResult; justCreated: boolean } {
 ...
 */
function checkClaudeMd(projectDir: string, fix: boolean): SanityCheckResult {
 ...
 */
function checkDomainAlignment(projectDir: string): SanityCheckResult {
  ...
  const parts: string[] = [];
  if (missing.length > 0) parts.push(`missing docs: ${missing.join(', ')}`);
  if (extra.length > 0) parts.push(`orphaned docs: ${extra.join(', ')}`);
 ...
 */
async function checkMapStaleness(
  projectDir: string,
  pluginData: string,
  fix: boolean,
  skipExpensive: boolean,
  justCreatedChangesMd: boolean,
  nodeModulesAvailable: boolean
): Promise<SanityCheckResult> {
 ...
 */
function checkDomainStaleness(projectDir: string): SanityCheckResult {
 ...
 */
function checkRecentActivity(projectDir: string, fix: boolean): SanityCheckResult {
...
function formatReport(report: SanityReport, quiet: boolean): string {
  const lines: string[] = [];
